...
const AdminUI = {
  ...
  ICONS: {
    ...
    conferences:`<svg class="admin-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8"/><path d="M12 17v4"/><path d="M7 10h10"/><path d="M7 7h4"/></svg>`,
  ...
  buildSidebarHTML(activePage, user) {
    ...
    const biblio  = ['conferences'].some(can);
    ...
    return `
      ...
      <nav class="admin-nav" aria-label="Navigation administration">
        ...
        ${biblio ? `
        <div class="admin-nav-section">
          <div class="admin-nav-section-title">Bibliothèque</div>
          ${navLink('conferences', B+'admin/bibliotheque/conferences.html', I.conferences, 'Conférences')}
        </div>` : ''}
        ${superSection ? `
        <div class="admin-nav-section">
